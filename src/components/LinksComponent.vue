...
const currentLinkName = computed({
  get: () => {
    return showEditLinkForm.value >= 0
        ? links.value[showEditLinkForm.value].name
        : newLinkName.value;
  },
  set: (newValue: string) => {
...
const currentLinkUrl = computed({
  get: () => {
    return showEditLinkForm.value >= 0
        ? links.value[showEditLinkForm.value].url
        : newLinkUrl.value;
  },
  set: (newValue: string) => {
    if (showEditLinkForm.value >= 0) {
...
<template>
  <div class="relative">
    <div class="relative ml-4">
      ...
      <ul v-if="showDropDown" tabindex="0" @click="showDropDown = false" @click.away.stop = "showDropDown = false" class="absolute z-10 bg-white py-2 mt-2 w-64 rounded-lg shadow-lg" style="right: 0;">
        <li v-for="(link, index) in links" :key="index" class="px-4 py-2">
          <a :href="link.url" rel="noopener noreferrer" class="text-blue-500">{{ link.name }}</a>
    ...
    <div class="relative">
      <form
          v-if="showAddLinkForm || showEditLinkForm >= 0"
      ...
      >
        <div class="mb-4">
          ...
          >
            {{ showEditLinkForm >= 0 ? "Update" : "Add" }}
          </button>
          <button
              type="button"
              @click="showAddLinkForm = false; showEditLinkForm = -1; editName = ''; editUrl = ''"
